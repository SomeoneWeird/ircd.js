...
Commands.prototype = {
  ...
  USER: function(user, username, hostname, servername, realname) {
    ...
    for(var i = 0; i < this.server.bans.length; i++) {
      var ban = this.server.bans[i];
      if(ban.hostmask == servername) {
        if(ban.expires === -1 || ban.expires > Date.now()) {
          user.send(this.server.host, irc.errors.youAreBanned, user.nick, ':You are banned (Reason: ' + ban.reason + ')');
          user.closeStream();
          return;
        } else {
          // ban has expired
          this.server.bans.splice(i, 1);
        }
      }
    }
  ...
  MODE: function(user, target, modes, arg) {
    ...
    if (this.server.channelTarget(target)) {
      var channel = this.server.channels.find(target);
      if (!channel) {
        user.send(this.server.host, irc.errors.noSuchChannel, target, ':No such channel.');
      } else if (modes) {
        if (modes[0] === '+') {
  ...
  WALLOPS: function(user, text) {
    ...
    if (!text || text.length === 0) {
      user.send(server.host, irc.errors.needMoreParams, user.nick, ':Need more parameters');
      return;
    }

    server.users.registered.forEach(function(user) {
      if (user.modes.indexOf('w') !== -1) {
        user.send(server.host, 'WALLOPS', ':OPERWALL - ' + text);
      }
    });
  },
  ...
  KLINE: function(user, nick, seconds, reason) {

    // TODO: check for duplicate bans, update them?

    if(!user.localOper) {
      return;
    }

    var expires = seconds > 0 ? Date.now() + (seconds * 1000) : -1;

    var targetUser = this.server.users.find(nick);
    ...
    var ban = {
      hostmask: targetUser.hostname,
      expires:  expires,
      reason:   reason || "You are banned."
    }

    this.server.bans.push(ban);

    user.quit("User has been K-Lined.");
