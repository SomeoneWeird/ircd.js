...
Server.prototype = {
  ...
  joinDefaultChannels: function(user) {

    if(this.config.joinOnConnect) {
      for(var channel in this.config.joinOnConnect) {
        var key = this.config.joinOnConnect[channel];
        this.channels.join(user, channel, key);
      }
    }
